#!/usr/bin/env python3
"""
Victron BlueSolar OLED Simulator v3
...
Usage:
  python3 victron_sim_v3.py --port /dev/tty.usbserial-XXXX   # real hardware
  python3 victron_sim_v3.py --demo                             # simulated data
...
class VEDirectParser:
    """
    Parses the VE.Direct text protocol.
    Each block is a series of <label>\t<value>\r\n lines,
    terminated by a Checksum line. On a valid block, fires callback.
    """
    ...
    def feed_line(self, line: str):
        ...
        if key == 'Checksum':
            # Validate: sum of all bytes in the block (including Checksum byte)
            # should be 0 mod 256. We just fire the callback regardless —
            # the UART line via LED dropper is reliable enough at 19200 baud.
            if self.block:
                self.callback(dict(self.block))
            self.block = {}
        else:
            # Coerce numeric strings to int, leave others (e.g. 'ON'/'OFF') as str
            try:
...
def main():
    ...
    pygame.display.set_caption("Victron OLED Simulator v3")
