...
Settings are read/written via HEX protocol.
...
# ─────────────────────────────────────────────
#  TEXT protocol charge state map
# ─────────────────────────────────────────────
CS_STATES = {
    "0":   "Off",
    "2":   "Fault",
...
#  Used for settings read/write only.
...
REGISTERS = [
    ...
    # History
    Register(0xEE10, "Yield Total",          "Total yield (resettable)",              "kWh",  0.01,   "R",  "d",  4),
    Register(0xEE11, "Yield Today",          "Yield today",                           "kWh",  0.01,   "R",  "d",  4),
    Register(0xEE12, "Max Power Today",      "Maximum power today",                   "W",    1,      "R",  "d",  2),
    Register(0xEE13, "Yield Yesterday",      "Yield yesterday",                       "kWh",  0.01,   "R",  "d",  4),
    Register(0xEE14, "Max Power Yesterday",  "Maximum power yesterday",               "W",    1,      "R",  "d",  2),
]
...
def read_text_frame(ser: serial.Serial, timeout: float = 15.0) -> Optional[dict]:
    ...
    Waits for a Checksum line to confirm frame is complete.
    """
    frame = {}
...
def parse_text_live(frame: dict) -> list:
    ...
    Returns list of (label, value, unit) tuples.
    """
    rows = []
    ...
    if 'CS' in frame:
        cs = frame['CS']
        label = CS_STATES.get(cs, f"Unknown ({cs})")
        rows.append(("Charge State",    label, ""))
    if 'MPPT' in frame:
        m = frame['MPPT']
        label = MPPT_STATES.get(m, f"Unknown ({m})")
        rows.append(("MPPT State",      label, ""))
    if 'ERR' in frame:
        e = frame['ERR']
        label = ERR_CODES.get(e, f"Error {e}")
        rows.append(("Error",           label, ""))

    # Load
    if 'LOAD' in frame:
        rows.append(("Load Output",     frame['LOAD'], ""))
    if 'IL' not in frame and 'I' in frame:
        pass  # load current only on models with load output
